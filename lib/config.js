...
/** The API key, or "" when the line is blank or still holds the placeholder. */
function readGeminiKey(value) {
  const key = String(value || DEFAULTS.GEMINI_API_KEY).trim();
  return key === DEFAULTS.GEMINI_API_KEY_PLACEHOLDER ? "" : key;
}

// Kept here rather than imported from lib/provider.js: that module reads the
// config, so importing it back would be a circle.
const KNOWN_PROVIDERS = ["ollama", "gemini"];
...
function loadConfig(env = process.env) {
  ...
  return {
    ...
    geminiApiKey: readGeminiKey(env.GEMINI_API_KEY),

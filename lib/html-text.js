...
/* The accents an entity name can put on a letter, as combining marks. */
const ACCENT_MARKS = {
  grave: "̀",
  acute: "́",
  circ: "̂",
...
function decodeEntities(text) {
  return text.replace(/&(#x?[0-9a-f]+|[a-z][a-z0-9]*);/gi, (whole, body) => {
    ...
    // The accented letters are all built the same way — &eacute;, &Ouml;,
    // &ntilde; — so they are composed rather than listed.
    const accented = /^([A-Za-z])(grave|acute|circ|tilde|uml|ring|cedil|slash)$/.exec(body);

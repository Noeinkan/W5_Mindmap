...
function createGeminiClient(config, fetchImpl = fetch) {
  ...
  async function callModel(targetModel, body) {
    ...
    if (!response.ok) throw await responseError(response, "Gemini request failed", "gemini_failed");
  ...
  async function generate(prompt, schema) {
    ...
    return tryModels(
      [model, ...config.geminiFallbackModels],
      async (targetModel) => {
        const data = await callModel(targetModel, body(targetModel));
        // An empty answer comes back as an empty answer, not as an error. Thrown,
        // lib/extract.js would read it as the network going down and stop the whole
        // run; returned, it is one chunk that gets re-asked and, failing that,
        // skipped with a warning — and `empty` is what that warning shows as the
        // reason (a cut-off answer, a blocked prompt).
        const text = readText(data);
        return text
          ? { response: text, model: targetModel }
          : { response: "", model: targetModel, empty: describeEmptyAnswer(data) };
      },
